...
class Store(db.Model):
    ...
    user_id = db.Column(db.Integer, db.ForeignKey('users.id'), nullable=False, default=5)
    
    user = db.relationship('User', foreign_keys=[user_id], backref=db.backref('owned_stores', lazy=True))
    clerks = db.relationship('User', foreign_keys=[User.store_id], backref=db.backref('clerks_in_store', lazy=True, uselist=True))
